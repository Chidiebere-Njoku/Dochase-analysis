...
df = pd.read_csv("Deals_2026_04_29.csv")

print("Python Environment Successfully Integrated with Quarto")
print("------------------------------------------------------")
...
cat("Amount missing  :", sum(is.na(df_raw$Amount)), "records\n")
cat("Date format     :", as.character(df_raw$`Closing Date`[1]), " (MM/DD/YYYY)\n")
```
...
cat("Date range        :",
    format(min(df$Closing_Date, na.rm = TRUE), "%d %b %Y"), "to",
    format(max(df$Closing_Date, na.rm = TRUE), "%d %b %Y"), "\n")
cat("Amount range (NGN):",
    format(min(df$Amount), big.mark = ","), "to",
    format(max(df$Amount), big.mark = ","), "\n")
```
...
cat("Upper fence (Q3 + 1.5*IQR): NGN",
    format(round(upper_fence), big.mark = ","), "\n")
cat("Outliers retained:", nrow(outliers), "\n")
...
normality_tbl |>
  kable(
    caption     = "Table 5: Anderson-Darling Normality Test by Stage (n ≥ 7)",
    col.names   = c("Stage", "n", "AD Statistic", "p-value", "Normal? (p > 0.05)"),
    digits      = 4,
    align       = c("l", "r", "r", "r", "c")
  )
```
...
cat("\nEta-squared (eta2) =", eta_sq, "->", effect_size, "practical effect\n")
```
...
sig_idx <- which(dunn_res$P.adjusted < 0.05)
cat("Significant Bonferroni-adjusted pairwise comparisons:\n")
if (length(sig_idx) > 0) {
  cat(paste0("  \u2022 ", dunn_res$comparisons[sig_idx]), sep = "\n")
} else {
  cat("  None survive Bonferroni correction (conservative for 36 pairs).\n")
}
...
lm2 <- lm(Log_Amount ~ Stage_Rank + Deal_Age_Days + Deal_Owner_f,
           data = df_reg)
cat("=== Model 2: OLS — log(1 + Amount) [Preferred] ===\n")
print(summary(lm2))
```
...
With more time and data, the following extensions would materially strengthen this analysis:

-   **Logistic regression** on the binary Closed Won / Closed Lost outcome — enabling win-probability scoring for every active deal in the pipeline.
-   **Survival analysis** (Kaplan-Meier + Cox PH) to formally model time-to-close by stage, owner, and deal size — addressing the temporal dimension more rigorously.
-   **Integration with campaign delivery data** — linking pipeline outcomes to actual media impressions, reach, and ROI metrics to measure the quality of won deals, not just their face value.
-   **Bayesian hierarchical modelling** for robust owner-level inference with partial pooling across the unequally-sized owner sub-groups.
-   **Enriched CRM fields** — adding campaign type and client industry at entry would immediately improve model R² and management decision granularity.
